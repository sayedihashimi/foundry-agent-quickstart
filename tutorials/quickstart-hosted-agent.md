...
The agent uses four tool functions — `SearchHotels`, `GetHotelDetails`, `CheckAvailability`, and `BookRoom`. These are static methods decorated with `[Description]` attributes that the LLM can invoke during a conversation.

The full source for `HotelTools.cs` is available in the [source repository](https://github.com/sayedihashimi/foundry-agent-quickstart/blob/main/src/sample-hosted-agent/SeattleHotelAgent.Hosted.Agent/Tools/HotelTools.cs). Copy that file into the *Tools* folder in the agent project.
